...
class Analyze:
    def collect_jsons_for_sweep(
        self, log_dir: str, do_not_return_preds: bool = True
    ) -> pd.DataFrame:
    ...
    def general_plotter(
        ...
        f, axs = plt.subplots(
            n_rows, n_subplots_per_row, figsize=(4 * n_subplots_per_row, 4 * n_rows)
        )
        avg_f, avg_axs = plt.subplots(
            n_rows, n_subplots_per_row, figsize=(3 * n_subplots_per_row, 4 * n_rows)
        )
        if n_rows == 1:
            axs = [axs]
            avg_axs = [avg_axs]
...
class BaselineGeographicShiftTable:
    REGIONS = ["Asia", "Europe", "Americas", "Africa", "SouthEastAsia", "WestAsia"]

    def __init__(self, baselines: pd.DataFrame):
    ...
    def make_table(self) -> pd.DataFrame:
        ...
        ].sort_values(by="dsetname")[["dsetname", "method", "accuracy", "worst region"]]
        return table
