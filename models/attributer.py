    ...
    texts_by_subpop_by_class = dict(
        {
            classname: dict(
                {
                    subpop_caption: [
                        template.format(subpop_caption)
                        for template in vlm_prompt_templates
                    ]
...
class GroundTruths(Attributer):
    def __init__(self, dset: ClassificationDset):
        assert (
            dset.has_gt_attrs
        ), f"{dset.dsetname} does not have ground truth attributes, \
    ...
    def infer_attrs(self, classnames: List[str]) -> Dict[str, List[str]]:
        return dict(
            {
                classname: self.dset.gt_attrs_by_class(classname)
...
class LLMBased(Attributer):
    ...
    def infer_attrs(self, classnames: List[str]) -> Dict[str, List[str]]:
        ...
        if os.path.exists(cache_path):
            ...
            assert (
                self.llm_query.question == dat["llm_prompt"]
            ), "Attempting to use cached"
            ...
            answers = dict(
                {
                    classname: self.llm.answer_questions(
                        [self.llm_query.question.format(classname)]
                    )[0]
            ...
            save_dict = dict(
                {"answers": answers, "llm_prompt": self.llm_query.question}
            )
            cache_data(cache_path, save_dict)

        return dict(
            {
                classname: self.llm.parse_answer(answers[classname])
                for classname in classnames
            }
        )
...
class Waffle(Attributer):
    """
    Based on
    https://github.com/ExplainableML/WaffleCLIP/blob/master/waffle_tools.py#L229

    Args:
        waffle_count set to default of 15 based on waffle codebase.
            this means descriptions are 15 random word + 15 random characters
    """

    def __init__(self, waffle_count: int = 15):
        try:
            self.word_list = pkl.load(open("waffle_word_list.pkl", "rb"))
        except FileNotFoundError:
            self.word_list = pkl.load(open("models/waffle_word_list.pkl", "rb"))

        self.waffle_count = waffle_count

    def wordify(self, string: str):
    ...
    def infer_attrs(self, classnames: List[str]) -> Dict[str, List[str]]:
        avg_num_words = int(
            np.max(
                [
                    np.round(
                        np.mean([len(self.wordify(x).split(" ")) for x in classnames])
                    ),
                    1,
                ]
            )
        ...
        avg_word_length = int(
            np.round(
                np.mean(
                    [
                        np.mean([len(y) for y in self.wordify(x).split(" ")])
                        for x in classnames
                    ]
                )
            )
        )
        word_list = [x[:avg_word_length] for x in self.word_list]

        num_spaces = (
            int(
                np.round(
                    np.mean([np.sum(np.array(list(x)) == " ") for x in classnames])
                )
            )
            + 1
        )
        num_chars = int(
            np.ceil(
                np.mean([np.max([len(y) for y in x.split(" ")]) for x in classnames])
            )
        )
        ...
        for class_name in classnames:
            for _ in range(self.waffle_count):
                base_word = ""
                for a in range(avg_num_words):
                    base_word += np.random.choice(word_list, 1, replace=False)[0]
                    if a < avg_num_words - 1:
                        base_word += " "
                descriptions[class_name].append(base_word)
                noise_word = ""
                for c in sample_key:
                    if c != " ":
                        noise_word += np.random.choice(
                            WAFFLE_CHARACTER_LIST, 1, replace=False
                        )[0]
                    else:
                        noise_word += ", "
                descriptions[class_name].append(noise_word)
        return descriptions
