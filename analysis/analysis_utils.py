    ...
    for c1 in class_indices:
        ...
        for c2 in class_indices:
            ...
            if c2 >= c1: 
                ...
                mat[cls1][cls2] = pd.DataFrame(s.cpu(),index=subpops_names_cl1,columns=subpops_names_cl2)
                mat[cls2][cls1] = pd.DataFrame(s.transpose(1,0).cpu(),index=subpops_names_cl2,columns=subpops_names_cl1)
    return mat 
...
def return_df_C_by_CK(dset, llm, vlm, llm_answers, vlm_prompts):
    ...
    return pd.concat(mat,axis=0,ignore_index=True,keys=mat.keys()) 
