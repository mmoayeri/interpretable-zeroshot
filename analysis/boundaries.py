...
def return_df_CK_by_CK(dset, vlm):
    ...
    attributer = init_attributer('groundtruth', dset, llm=None)

    texts_by_subpop_by_class = infer_attrs(dset.classnames, [attributer], ['A photo of a {}'])
    ...
    text_embeddings_by_cls, subpops = vlm_handler.convert_to_embeddings_by_cls(text_embeddings_by_subpop_by_cls)
    ...
    for c1 in range(len(dset.classnames)):
        ...
        for c2 in range(len(dset.classnames)):
            ...
            s = cossim(text_embeddings_by_cls[cls1].unsqueeze(1), text_embeddings_by_cls[cls2].unsqueeze(0))
...
def main(args):
    ...
    # df = pd.concat([df,df_target],axis=1)
        ...
        df_confusion = dset.data_df['attr'].isin([str1.split(',')[0],str2.split(',')[0]])
